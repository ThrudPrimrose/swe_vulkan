...
void Device::createFramebuffers() {
 ...
 VkShaderModule Device::createShaderModule(const std::vector<char>& code) {
    ...
    QueueFamilyIndices Device::findQueueFamilies(VkPhysicalDevice device,VkSurfaceKHR surface) {
        ...
        qFamilyInd = (indices.graphicsFamily).value();
        return indices;
    }

    std::vector<const char*> Device::getRequiredExtensions(bool enableValidationLayers) {
    ...
    bool Device::checkValidationLayerSupport(bool enableValidationLayers, const std::vector<const char*> validationLayers) {
        uint32_t layerCount;
