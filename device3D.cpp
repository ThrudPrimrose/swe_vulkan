...
void Device3D::createGraphicsPipeline() {
        ...
        colorBlendAttachment.srcColorBlendFactor=VK_BLEND_FACTOR_SRC_ALPHA; //was SRC_COLOR
        colorBlendAttachment.dstColorBlendFactor=VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA; //was src_color
...
void Device3D::createImageViews() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            if (vkCreateImageView(logicDevice, &createInfo, nullptr,
...
void Device3D::initArrays(){
    ncReader3D.readAndInit("netcdfreader/tohoku.nc");
...
VkFormat Device3D::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
VkFormat Device3D::findDepthFormat() {
...
bool Device3D::hasStencilComponent(VkFormat format) {
    return format == VK_FORMAT_D32_SFLOAT_S8_UINT || 
    format == VK_FORMAT_D24_UNORM_S8_UINT;
}

void Device3D::createDepthResources(){
    VkFormat depthFormat = findDepthFormat();
    
    imageCreator.createImage(swapChainExtent.width, swapChainExtent.height, 
    msaaSamples,
    depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, 
    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory,
    logicDevice,physicalDevice);
...
VkImageView Device3D::createImageView(VkImage image, VkFormat format,
...
void Device3D::createRenderPass() {
        ...
        //if this is the final-> change back to 
        //was VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
...
void Device3D::createFramebuffers() {
...
void Device3D::cleanupSwapChain() {
        vkDestroyImageView(logicDevice, colorImageView, nullptr);
        vkDestroyImage(logicDevice, colorImage, nullptr);
        vkFreeMemory(logicDevice, colorImageMemory, nullptr);
...
VkSampleCountFlagBits Device3D::getMaxUsableSampleCount() {
...
void Device3D::setMaxUsableSampleCount(){
    msaaSamples=getMaxUsableSampleCount();
    std::cout<<"Max Usable Sample Count: "<<msaaSamples<<std::endl;

}

void Device3D::createMSAAResources() {
